module Api
  module Friends
    class FriendRequestsController < Api::Friends::ContextController
      ...
      private
		...
		def ensure_friend_request_permission
		  render_permission_error unless [@friend_request.friend_request_user_id, @friend_request.user_id].include?(current_user._id)
		end

		def ensure_friend_request_sent_to_current_user
		  render_permission_error unless @friend_request.friend_request_user_id === current_user._id
		end

		def ensure_current_user_owns_friend_request
		  render_permission_error unless @friend_request.user_id === current_user._id
		end
